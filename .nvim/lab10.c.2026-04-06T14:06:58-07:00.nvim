...
Uses AF_INET for ipv4 address, and SOCK_STREAM for TCP

2. What data is the client sending to the server?
It is sending pre defined strings to the server

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
Passes a struct containing a clients socket file descriptor, a lock for the
client, and a list handle struct

2. How are received messages stored?
Messages are stored in a linked list struct called list_node

3. What does `main()` do with the received messages?
Ensures all the recieved messages from client are collected, removes them from
lsit_node linked list

4. How are threads used in this sample code?
threads are used to accept a client and store data tramsitted from client

5: How are sockets made non blocking? using fcnt() to set O NONBLOCK flag
All sockets made non blocking
Socket are made non blocking to manage mutliple clients simultaneously without
stalling/being blocked
*/
